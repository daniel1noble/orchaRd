...
pacman::p_load(devtools, tidyverse, metafor, patchwork, R.rsp, orchaRd, emmeans, ape, phytools)
```
...
#### Heterogeneous variance models at multiple levels

```{r}
rm(list = ls())
load("./MS/misc/processed_data.RData")
```
...
orchard_plot(mod.habitat_het, data = d, group = "species_ID", mod = "habitat", xlab = "dARR", angle = 45)
```
