...
In `orchaRd` 2.0 we've made some major updates to the types of models that can be used with the package. Meta-analyses in ecology and evolution often fit a multitude of moderators and random effects within a single model. This is even desiredable in many cases to deal with high heterogeneity and disentangle the independent contributions of modeators to changes in mean effect size. The challenge with previous versions of the package was that it was difficult to accomodate these models. However, the `emmeans` packages is an excellent solution to the problem. It will produce marginsalised mean estimates for a given level of moderator -- producing mean estimates that average over all other moderators in the model. This includes models with categorical and continuous moderators. 

An additional issue that is of importance with meta-regerssion models is the need to deal with heteroscedastic variance across levels of a moderator. Homogeneity of variance assumptions are often violated in meta-analysis. `orchaRd` 2.0 allows the user to fit models that explicitly estaimte different residual variance reducing the probability of type I errors. 

In this section, we show users: 1) how to fit these models using `metafor` and 2) how these models intrface with the `orchaRd` package to produce pretty plots that capture the model predictions. 
...
We'll work with a meta-analysis by O'Dea et al. 2019. In this meta-analysis, the authors were interested in testing whether developmental temperaure in fish had an effect on mean and variance in a host of phenotypic traits. They categorised effect sizes as having come from one of 4 trait categories: Behaviour, Life-history, Morphology and Physiology. We'll explore how to fit a meta-regression model that explictly estimates residual variance in each of these trait levels. 
...
Now that we have fit our model and estimated the relevant parameters we can produce the orchard plot  using the `mod_results` function.Figure \@ref(fig:Figure11)

```{r figure 12, fig.cap = "Orchard plot with heteroscedastic error for trait typess", echo = TRUE, eval = TRUE, warning=FALSE, message=FALSE, class.source='klippy'}
...
For this example, we turn to a fish meta-analysis conducted by O'Dea et al. 2019. In this analysis a suite of different moderators were of interst. 
...
Now that we have the data loaded we can fit a few meta-regression models. For now, we'll keep the model simple. We'll assume that the variance in the moderator of interst is the same across levels. We'll relax that assumption down the line.
...
Here, we are modelling the log response ratio effect size (`lnrr`) controlling for the fact that many effect sizes come from the same study (`group_ID`) We're controlling for trait type, exerimental design, and degree difference between treatments and how long the treatments were applied. 
