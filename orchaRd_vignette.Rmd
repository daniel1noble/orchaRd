...
In addition to orchard plots, we have another plotting function `caterpillars` to draw caterpillar(s) plots. Furthermore, the pacakge `orchaRd` comes with two functions that are useful for multilevel meta-analysis and meta-regression (i.e., `rma.mv`): 1) `i2_ml` to calculate the 'total' I^2^ along with I^2^ for each level (*sensu* Nakagawa & Santos 2012; see also ; Senior et al. 2016) and 2) `r2_ml` to calcuate marginal and conditional R^2^ for mixed models (*sensu* Nakagawa & Schielzeth 2013). 
...
eklof_MR_result <- orchaRd::mod_results(eklof_MR, mod = "Grazer.type", data = eklof, group = "ExptID")
...
HetModel2 <- orchaRd::marginal_means(model, group = "group_ID", mod = "trait.type", at = list(deg_dif = c(5, 10, 15), treat_end_days = c(10)), by = "deg_dif", weights = "prop", data = warm_dat)
...
Now, the orchard plot depicts the mean effect sizes (plus 95% CI and PI) for each trait category, at 5, 10 and 15 degrees Celsius when the treatment duration is 10 days. 
