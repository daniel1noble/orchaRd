...
By default, `orchard_plot()` capitalises the first letter of each moderator level. Set `upper = FALSE` to preserve the original case. This is also compatible with `tree.order`, which lets you reorder categories in the plot. When `upper = FALSE`, pass `tree.order` values in the same case as your data (@fig-upperlower):
...
orchaRd::orchard_plot(lim_lower_MR, mod = "Phylum", group = "Article",
  xlab = "Correlation coefficient (r)", transfm = "tanh", N = "N",
  upper = FALSE, tree.order = my_order, angle = 45)
```
