...
<img src="man/figures/logo.png" alt="orchaRd hex sticker" style="float:right; width:170px; margin:0 0 1em 1.5em;"/>
...
`orchaRd` is on CRAN, so most users should install the released version with the code below.


:::{.column-margin}
:::{.callout-note}
If you want the development version, which may contain newer features not yet on CRAN, you can install it from GitHub instead:
...
remotes::install_github("daniel1noble/orchaRd", ref = "dev")
pacman::p_load(tidyverse, metafor, patchwork, R.rsp, orchaRd, emmeans, ape, phytools, flextable, rotl, clubSandwich, glmmTMB, lme4)
```

It's important to note that this version may not be as stable as the CRAN release.

:::
:::

```{r echo = TRUE, eval = FALSE, results = 'hide', warning=FALSE, message=FALSE,  class.source='klippy'}
# Install the released version of orchaRd from CRAN
install.packages("orchaRd")

# Load orchaRd together with the other packages used throughout this vignette
# install.packages("pacman")
pacman::p_load(orchaRd, tidyverse, metafor, patchwork, R.rsp, emmeans, ape, phytools, flextable, glmmTMB, lme4)
```

Installing `orchaRd` from CRAN will make the primary functions accessible to users along with their help files, and will automatically install its required dependencies (including `metafor`). The examples in this vignette also make use of the `tidyverse` and `metafor` packages.
